...
### 当前快照（v10，2026-09-10 真实重跑）

| 评测集 | 题量 | 结果正确率 | SQL 可执行率 |
|---|---|---|---|
| **基础集（baseline）** | 60 | **76.7%（46/60）** | 100%（60/60）|
| **进阶集（advanced）** | 20 | **40.0%（8/20）** | 100%（20/20）|
| **总集（total）** | 80 | **67.5%（54/80）** | 100%（80/80）|
...
|---|---|---|---|---|
| 真实 LLM **v10**（当前） | 80 | **67.5%** | 100% | `deepseek-chat`、temperature=0、当前代码全量重跑；单轮结果，不宣称稳定提升 |
| 真实 LLM v9 | 80 | 64.0% | 98.8% | 撤回 rule 14（JOIN 完整性）+ 首跑 60/20 拆分 |
...
### 关键诚实声明
1. **单次 LLM 生成完整 SQL 的历史 80 题结果集中在 62–65%，本次真实重跑为 67.5%**。这里只报告单轮快照；在同一协议下重复多轮前，不把 3.5 个百分点差异宣称为稳定提升。v4 的 95% 是 20 题过拟合，不能作为项目成绩。
2. **60/20 拆分后真相更清晰**：当前基础 60 题 76.7%，进阶 20 题仅 40.0%。复杂题仍需查询计划与多步分解，继续堆叠 Prompt 不是可靠解法。
...
完整报告：
- 真实评测：`eval/report-real-current-20260910.md`（当前）+ `eval/sql_dump-real-current-20260910.jsonl`（逐题 SQL 与差异）；历史轮次见 `docs/evaluation-history.md`
